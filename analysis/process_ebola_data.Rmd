...
raw_p <- imap(by_location, function(x, cntry) {
  x <- select(x, issue_date, sc, cc, country) %>%
    gather(
      case_type,
...
cum_cases <- map(by_location, function(df) {
...
```{r data-cleanup-10 }
no_dups <- map(cum_cases, function(df) {
...
total_p <- map(total_p, function(p) {
...
nodups_p <- map(nodups_p, function(p) {
        p <- p + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
        p
})     
```
...
nooutss_p <- map(nooutss_p, function(p) {
        p <- p + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
        p
})      
```
...
monoinc_p <- map(monoinc_p, function(p) {
...
interp_p <- map(interpolated, function(x) {
  x$src <- glue("{params$datasource}.FALSE")
  idx <- which(is.na(x$cases))
  x$src[idx] <- glue("{params$datasource}.TRUE")
  x <- select(x,
    date = interpolated_date,
    cases = interpolated_cases,
    src
