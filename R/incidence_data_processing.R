...
is_outlier <- function(cases, use_last, k_sd) {
  ...
  p_interval <- prediction_interval(
    p_interval[-1],
    k_sd
  )
...
make_increasing_at_k <- function(df, t1) {
  t2 <- t1 + 1
  if (nrow(df) == t2) {
    df <- make_increasing_at_end(df, t1)
  } else {
    df_copy <- df
    df_no_t1 <- df_copy
    df_no_t1 <- df_no_t1[-t1, ]
    is_increasing_no_t1 <- is_monotonically_increasing(df_no_t1$cases[c(
      t1 - 1,
      t2 - 1
    )])
    ...
    is_increasing_no_t2 <- is_monotonically_increasing(df_no_t2$cases[c(
      t1 - 1,
      t2 - 1
    )])
